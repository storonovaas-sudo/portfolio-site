// Local, framework-free adaptation of React Bits LightRays by David Haz.
...
(async () => {
  ...
  if (!gl) return;
  const response = await fetch('previews/light-rays/shader.frag');
  ...
  resize();
})().catch(error => console.warn('Light Rays preview:', error));
